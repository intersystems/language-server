			...
			if (params.position.character >= symbolstart && params.position.character <= symbolend) {
				...
				if (
					...
					(parsed[params.position.line][i].l == ld.cls_langindex &&
						parsed[params.position.line][i].s == ld.cls_param_attrindex)
				) {
					// Only save the symbol range if it's potentially renameable
					result = Range.create(
						Position.create(params.position.line, symbolstart),
						Position.create(params.position.line, symbolend),
					);
			...
			if (
				...
				if (
					...
					for (let l = 1; l < parsed[params.position.line].length; l++) {
						if (
							parsed[params.position.line][l].l == ld.cls_langindex &&
							parsed[params.position.line][l].s == ld.cls_keyword_attrindex
						) {
							const kw = doc.getText(
								Range.create(
									...
									Position.create(
										params.position.line,
										parsed[params.position.line][l].p + parsed[params.position.line][l].c,
									),
								),
							);
								...
								if (
									doc.getText(
										Range.create(
											...
											Position.create(
												params.position.line,
												parsed[params.position.line][l + 1].p + parsed[params.position.line][l + 1].c,
											),
										),
									) === "="
								) {
									...
									const kwval = doc.getText(
										Range.create(
											...
											Position.create(
												params.position.line,
												parsed[params.position.line][l + 2].p + parsed[params.position.line][l + 2].c,
											),
										),
									);
									if (kwval === "0") {
						...
						if (
							...
							for (let l = 1; l < parsed[mline].length; l++) {
								if (parsed[mline][l].l == ld.cls_langindex && parsed[mline][l].s == ld.cls_keyword_attrindex) {
									const kw = doc.getText(
										Range.create(
											Position.create(mline, parsed[mline][l].p),
											Position.create(mline, parsed[mline][l].p + parsed[mline][l].c),
										),
									);
									if (kw.toLowerCase() === "procedureblock") {
										// The ProcedureBlock keyword is set
					...
					for (let line = 0; line <= params.position.line; line++) {
						...
						if (
							...
							for (let ctkn = 2; ctkn < parsed[line].length; ctkn++) {
								if (
									parsed[line][ctkn].l === ld.cls_langindex &&
									parsed[line][ctkn].s === ld.cls_keyword_attrindex &&
									doc
										.getText(
											Range.create(
									...
									// The ProcedureBlock keyword is set
									if (
										parsed[line][ctkn - 1].l === ld.cls_langindex &&
										parsed[line][ctkn - 1].s === ld.cls_keyword_attrindex &&
										doc
											.getText(
												Range.create(
...
export async function onRenameRequest(params: RenameParams) {
	...
	if (lang === ld.cos_langindex) {
		...
		for (let line1 = params.position.line - 1; line1 >= 0; line1--) {
				...
				if (type !== ld.cos_param_attrindex) {
					...
					for (let udltkn = 0; udltkn < parsed[line1].length; udltkn++) {
						if (parsed[line1][udltkn].l == ld.cls_langindex && parsed[line1][udltkn].s == ld.cls_param_attrindex) {
							...
							const udltknrange = Range.create(
								Position.create(line1, parsed[line1][udltkn].p),
								Position.create(line1, parsed[line1][udltkn].p + parsed[line1][udltkn].c),
							);
			...
			for (let tkn1 = 0; tkn1 < parsed[line1].length; tkn1++) {
				if (parsed[line1][tkn1].l == ld.cos_langindex && parsed[line1][tkn1].s == type) {
					...
					const tkn1range = Range.create(
						Position.create(line1, parsed[line1][tkn1].p),
						Position.create(line1, parsed[line1][tkn1].p + parsed[line1][tkn1].c),
					);
		...
		for (let line2 = params.position.line; line2 < parsed.length; line2++) {
			...
			for (let tkn2 = 0; tkn2 < parsed[line2].length; tkn2++) {
				if (parsed[line2][tkn2].l == ld.cos_langindex && parsed[line2][tkn2].s == type) {
					// This token is the same type as the one we're renaming
					const tkn2range = Range.create(
						Position.create(line2, parsed[line2][tkn2].p),
						Position.create(line2, parsed[line2][tkn2].p + parsed[line2][tkn2].c),
					);
		...
		for (let line3 = params.position.line; line3 < parsed.length; line3++) {
			...
			if (
				line3 !== params.position.line &&
				parsed[line3][0].l === ld.cls_langindex &&
				parsed[line3][0].s === ld.cls_keyword_attrindex
			) {
				break;
			}
			for (let tkn3 = 0; tkn3 < parsed[line3].length; tkn3++) {
				if (
					...
					// This token is the same type as the one we're renaming
					const tkn3range = Range.create(
						Position.create(line3, parsed[line3][tkn3].p),
						Position.create(line3, parsed[line3][tkn3].p + parsed[line3][tkn3].c),
					);
