...
function processMember(
	...
	line: number,
): { deprecated: boolean; lastNonEmpty: number } {
	...
	for (let nl = line; nl < parsed.length; nl++) {
		...
		if (nl > line && parsed[nl][0].l === ld.cls_langindex) {
			...
			if (parsed[nl][0].s === ld.cls_keyword_attrindex) {
				const nextkeytext = doc
					.getText(Range.create(nl, parsed[nl][0].p, nl, parsed[nl][0].p + parsed[nl][0].c))
					.toLowerCase();
		...
		if (deprecated == undefined) {
			const depTkn = parsed[nl].findIndex(
				(e) =>
					e.l == ld.cls_langindex &&
					e.s == ld.cls_keyword_attrindex &&
					doc.getText(Range.create(nl, e.p, nl, e.p + e.c)).toLowerCase() == "deprecated",
			);
			if (depTkn != -1) {
				...
				deprecated =
					...
					doc
						.getText(
							Range.create(
								previous[0],
								parsed[previous[0]][previous[1]].p,
								previous[0],
								parsed[previous[0]][previous[1]].p + parsed[previous[0]][previous[1]].c,
							),
						)
						.toLowerCase() != "not";
			}
...
export async function onDocumentSymbol(params: DocumentSymbolParams) {
	...
	if (doc.languageId === "objectscript-class") {
			...
			if (
				...
				if (keywordtextlower === "class") {
					...
					for (let nl = line - 1; nl >= 0; nl--) {
						...
						if (
							!parsed[nl]?.length ||
							(parsed[nl][0].l === ld.cls_langindex && parsed[nl][0].s !== ld.cls_desc_attrindex)
						)
							break;
					}
					...
					members.push({
						name: memName,
						kind: ["method", "classmethod", "clientmethod"].includes(keywordtextlower)
							? SymbolKind.Method
							: keywordtextlower == "query"
								? SymbolKind.Function
								: keywordtextlower == "trigger"
									? SymbolKind.Event
									: keywordtextlower == "parameter"
										? SymbolKind.Constant
										: keywordtextlower == "index"
											? SymbolKind.Array
											: keywordtextlower == "foreignkey"
												? SymbolKind.Key
												: keywordtextlower == "xdata"
													? SymbolKind.Struct
													: keywordtextlower == "storage"
														? SymbolKind.Object
														: keywordtextlower == "projection"
		...
		for (let line = 0; line < parsed.length; line++) {
			...
			const secondtokentext = doc
				.getText(Range.create(line, parsed[line][1].p, line, parsed[line][1].p + parsed[line][1].c))
				.toLowerCase();
			if (
				parsed[line][1].l === ld.cos_langindex &&
				parsed[line][1].s === ld.cos_ppc_attrindex &&
				(secondtokentext === "define" || secondtokentext === "def1arg")
			) {
				...
				if (
					...
					const fullrange: Range = Range.create(
						line - prevdoccomments,
						0,
						line,
						parsed[line][parsed[line].length - 1].p + parsed[line][parsed[line].length - 1].c,
					);
				...
				result.push({
					name: doc.getText(
						Range.create(
							multilinestart,
							parsed[multilinestart][2].p,
							multilinestart,
							parsed[multilinestart][2].p + parsed[multilinestart][2].c,
						),
					),
					kind: SymbolKind.Constant,
					range: Range.create(
						multilinestart - prevdoccomments,
						0,
						line,
						parsed[line][parsed[line].length - 1].p + parsed[line][parsed[line].length - 1].c,
					),
		...
		for (let line = 0; line < parsed.length; line++) {
			...
			if (
				...
				if (firstbrace != undefined) {
					...
					let brk = false;
					for (
						let nl = parsed[firstbrace[0]].length - 1 == firstbrace[1] ? firstbrace[0] + 1 : firstbrace[0];
						nl < parsed.length;
						nl++
					) {
				...
				if (inProcedureBlock) {
					...
					result[result.length - 1].children?.push({
						...
						range: Range.create(
							...
							Position.create(
								endLine,
								parsed[endLine][parsed[endLine].length - 1].p + parsed[endLine][parsed[endLine].length - 1].c,
							),
						),
		...
		for (let line = 0; line < parsed.length; line++) {
			...
			for (let tkn = 0; tkn < parsed[line].length; tkn++) {
				if (
					...
					let method: string = "";
					let name: string = "";
					let methodrange: Range = Range.create(0, 0, 0, 0);
					let namerange: Range = Range.create(0, 0, 0, 0);
					for (let stkn = tkn + 2; stkn < parsed[line].length; stkn++) {
						if (parsed[line][stkn].l == ld.html_langindex && parsed[line][stkn].s == ld.html_name_attrindex) {
							...
							const attrtext: string = doc
								.getText(Range.create(line, parsed[line][stkn].p, line, parsed[line][stkn].p + parsed[line][stkn].c))
								.toLowerCase();
							if (parsed[line].length > stkn + 2) {
								const valrange: Range = Range.create(
									line,
									parsed[line][stkn + 2].p,
									line,
									parsed[line][stkn + 2].p + parsed[line][stkn + 2].c,
								);
								let valtext: string = doc.getText(valrange);
					...
					if (
						...
						let detail: string = "ObjectScript";
				...
				if (
					...
					result[result.length - 1].range.end = Position.create(
						line,
