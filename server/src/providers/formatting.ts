...
async function formatText(uri: DocumentUri, range?: Range): Promise<TextEdit[] | null> {
	...
	if (range == undefined) {
		...
		range = Range.create(
			...
			0,
			lastnonempty,
			parsed[lastnonempty][parsed[lastnonempty].length - 1].p + parsed[lastnonempty][parsed[lastnonempty].length - 1].c,
		);
	}
		...
		if (doc.languageId === "objectscript-class") {
			...
			if (hassupers) {
				...
				if (pkgrespdata?.data?.result?.content?.length == 1) {
					// We got data back
					inheritedpackages =
						pkgrespdata.data.result.content[0].Importall != ""
	...
	for (let line = range.start.line; line <= range.end.line; line++) {
		...
		for (let token = 0; token < parsed[line].length; token++) {
			...
			if (parsed[line][token].l == ld.cos_langindex && parsed[line][token].s == ld.cos_command_attrindex) {
				...
				const commandrange = Range.create(
					Position.create(line, parsed[line][token].p),
					Position.create(line, parsed[line][token].p + parsed[line][token].c),
				);
				...
				if (commandtext.toUpperCase() === "H") {
					...
					commanddoc = commands.find(
						(el) => el.label === commandtext.toUpperCase() || el.alias.includes(commandtext.toUpperCase()),
					);
				}
				...
				const sysfrange = Range.create(
					Position.create(line, parsed[line][token].p),
					Position.create(line, parsed[line][token].p + parsed[line][token].c),
				);
				const sysftext = doc.getText(sysfrange);
				const sysfdoc = systemFunctions.find(
					(el) => el.label === sysftext.toUpperCase() || el.alias.includes(sysftext.toUpperCase()),
				);
				if (sysfdoc !== undefined) {
					...
					if (settings.formatting.system.case === "lower") {
						...
						} else if (idealsysftext === "$ISVECTOR") {
							idealsysftext = "$IsVector";
						...
						} else if (idealsysftext === "$VECTORDEFINED") {
							idealsysftext = "$VectorDefined";
				...
				if (ssysvtext !== "^$") {
					if (ssysvtext.indexOf("^$") === -1) {
						const ssysvdoc = structuredSystemVariables.find(
							(el) => el.label === "^$" + ssysvtext.toUpperCase() || el.alias.includes("^$" + ssysvtext.toUpperCase()),
						);
				...
				const sysvrange = Range.create(
					Position.create(line, parsed[line][token].p),
					Position.create(line, parsed[line][token].p + parsed[line][token].c),
				);
				const sysvtext = doc.getText(sysvrange);
				const sysvdoc = systemVariables.find(
					(el) => el.label === sysvtext.toUpperCase() || el.alias.includes(sysvtext.toUpperCase()),
				);
				if (sysvdoc !== undefined) {
					let idealsysvtext: string;
					if (settings.formatting.system.length === "short" && sysvdoc.alias.length === 2) {
						if (
							sysvtext.toUpperCase() === "$SYSTEM" &&
							parsed[line][token + 1].l == ld.cos_langindex &&
							parsed[line][token + 1].s == ld.cos_clsname_attrindex
						) {
				...
				parsed[line][token].l == ld.cos_langindex &&
				(parsed[line][token].s == ld.cos_uknzfunc_attrindex || parsed[line][token].s == ld.cos_uknzvar_attrindex)
			) {
				// This is an unknown Z function or variable

				const unknsrange = Range.create(
					Position.create(line, parsed[line][token].p),
					Position.create(line, parsed[line][token].p + parsed[line][token].c),
				);
				...
				(token == 0 ||
					(token > 0 &&
						(parsed[line][token - 1].l != parsed[line][token].l ||
							parsed[line][token - 1].s != parsed[line][token].s))) &&
				...
				if (
					...
					parsed[line][token - 1].s == ld.cls_keyword_attrindex
				) {
