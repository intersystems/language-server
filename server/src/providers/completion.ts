 ...
 */
export const schemaCaches: Map<ServerSpec, SchemaCache> = new Map();
 ...
 */
let macroCompletionCache: MacroContext;
...
class Schema {
	...
	private makeEntry(
		...
		attributes: string,
		offset: number,
		elemCollection: Map<string, Element>,
	) {
		let elemname: string;
		...
		if (slashidx === -1) {
			// This is a leaf
			const lastelem: string = elementPath.slice(offset);
			...
			let elem = elemCollection.get(elemname);
			if (elem !== undefined) {
	...
	convertI2E(intname: string): string {
		let namespaceIdx: string;
		let externalelemname: string;
		const delimpos = intname.indexOf(":");
		if (delimpos !== -1) {
			namespaceIdx = intname.substring(0, delimpos);

			// Find the namespace
			const ns = this.i2n.get(namespaceIdx);
			if (ns !== undefined) {
	...
	getMappings(): string {
		const result: string[] = [];
		for (const m of this.prefixMappings) {
...
class SchemaCache {
	...
	async getSchema(schemaurl: string): Promise<Schema | undefined> {
		let schema = this.schemas.get(schemaurl);
		if (schema === undefined) {
			...
			const respdata = await makeRESTRequest(
				"GET",
				2,
				"/saschema/" + schemaurl,
				this.server,
...
async function completionInclude(server: ServerSpec): Promise<CompletionItem[]> {
	...
	const respdata = await makeRESTRequest("POST", 1, "/action/query", server, querydata);
	if (respdata !== undefined && respdata.data.result.content.length > 0) {
		for (const incobj of respdata.data.result.content) {
			result.push({
			...
			if (parsed[ln][tkn].l == ld.cos_langindex && parsed[ln][tkn].s == ld.cos_delim_attrindex) {
				...
				if (delimtext == "(") {
					...
					if (
						...
						parsed[ln][tkn - 1].s == ld.cos_sysf_attrindex
					) {
						const sysf = doc
							.getText(Range.create(ln, parsed[ln][tkn - 1].p, ln, parsed[ln][tkn - 1].p + parsed[ln][tkn - 1].c))
							.toLowerCase();
						if (sysf == "$bitlogic") {
...
export async function onCompletion(params: CompletionParams): Promise<CompletionItem[] | null> {
	...
	if (parsed[params.position.line].length >= 2) {
		firsttwotokens = doc.getText(
			Range.create(
				params.position.line,
				parsed[params.position.line][0].p,
				params.position.line,
				parsed[params.position.line][1].p + parsed[params.position.line][1].c,
			),
		);
	}
	...
	if (analysis !== undefined && !("error" in analysis)) {
		...
		for (const member of analysis.members) {
			if (member.kind.isClassMethod() || member.kind.isClientMethod() || member.kind.isMethod()) {
				...
				if (isPositionBefore(method.body.lft, params.position) && isPositionBefore(params.position, method.body.rht)) {
					const methodBody = doc.getText(Range.create(method.body.lft, params.position));
		...
		// })
		const classnameOffset = completeClassResult?.classname ?? completeMethodResult?.classname;
		if (classnameOffset !== undefined) {
			if (classnameOffset === 0) {
				...
				const filter = doc.getText(
					Range.create(doc.positionAt(doc.offsetAt(params.position) - classnameOffset), params.position),
				);
		...
		const statementOffset = completeClassResult?.statement ?? completeMethodResult?.statement;
		if (statementOffset !== undefined && statementOffset <= 3) {
			const filter = doc.getText(
				Range.create(doc.positionAt(doc.offsetAt(params.position) - statementOffset), params.position),
			);
			for await (const item of completeStatment(filter)) {
				result.push(item);
			}
		}
		return result;
	...
	if (prevline.endsWith("$$$") && [ld.cos_langindex, ld.sql_langindex].includes(triggerlang)) {
							...
							for (let mln = ln + 1; mln < parsed.length; mln++) {
								if (
									...
									docstr =
										docstr +
										"\n" +
										doc.getText(Range.create(mln, 0, mln, parsed[mln][endTkn].p + parsed[mln][endTkn].c)).trimEnd();
								} else {
		...
		if (prevline.charAt(prevline.length - 2) === "^") {
			...
			for (const ssv of structuredSystemVariables) {
		...
		((triggerlang === ld.cos_langindex || triggerlang === ld.cls_langindex) &&
			(asRegex.test(prevlineLower) || /\s+of\s+$/.test(prevlineLower))) ||
		...
		const prevtokentext = doc.getText(prevtokenrange);
		if (
			(parsed[params.position.line][thistoken - 1].l == ld.cls_langindex &&
				parsed[params.position.line][thistoken - 1].s == ld.cls_clsname_attrindex) ||
			(parsed[params.position.line][thistoken - 1].l == ld.cos_langindex &&
				parsed[params.position.line][thistoken - 1].s == ld.cos_clsname_attrindex)
		) {
			// This is a class name
			const prevchar = doc.getText(
				Range.create(
					params.position.line,
					prevtokenrange.start.character - 1,
					params.position.line,
					prevtokenrange.start.character,
				),
			);
			...
			parsed[params.position.line][thistoken - 1].l == ld.cos_langindex &&
			parsed[params.position.line][thistoken - 1].s == ld.cos_sysv_attrindex &&
			prevtokentext.toLowerCase() === "$system"
		) {
			// This is $SYSTEM
			...
			prevline.endsWith(".") &&
			triggerlang === ld.cls_langindex &&
			parsed[params.position.line][thistoken].s == ld.error_attrindex
		) {
		...
		if (prevtokentype === "class" || prevtokentype === "system") {
			...
			if (prevline.slice(-2) === ".#") {
				...
				if (membercontext.context == "instance") {
					...
					data.query +=
						"SELECT Name, Description, Origin, FormalSpec, ReturnType AS Type, 'method' AS MemberType, Deprecated, NULL AS Aliases " +
					...
					// Non-generated methods
					data.query +=
						"SELECT Name, Description, Origin, FormalSpec, ReturnType AS Type, 'method' AS MemberType, Deprecated " +
		...
		const normalizedcls = await normalizeClassname(
			doc,
			parsed,
			determineClassNameParameterClass(doc, parsed, params.position.line, thistoken, true),
			server,
		...
		if (prevline.trimEnd().endsWith(",")) {
			...
			for (let tkn = thistoken; tkn >= 0; tkn--) {
				if (
					parsed[params.position.line][tkn].l == ld.cls_langindex &&
					parsed[params.position.line][tkn].s == ld.cls_delim_attrindex
				) {
					...
					parsed[params.position.line][tkn].l == ld.cls_langindex &&
					parsed[params.position.line][tkn].s == ld.cls_cparam_attrindex
				) {
					existingparams.push(
						doc.getText(
							Range.create(
		...
		(/\[\s*$/.test(prevline) ||
			(/,\s*$/.test(prevline) && openParenCount <= closeParenCount) ||
		...
		for (let i = 1; i < parsed[params.position.line].length; i++) {
			...
			if (
				...
				parsed[params.position.line][i].s == ld.cls_delim_attrindex
			) {
				switch (symboltext) {
					case "[":
		...
		for (let i = 1; i < parsed[params.position.line].length; i++) {
			...
			const symboltext = doc
				.getText(Range.create(params.position.line, symbolstart, params.position.line, symbolend))
				.toLowerCase();
			if (
				parsed[params.position.line][i].l == ld.cls_langindex &&
				parsed[params.position.line][i].s == ld.cls_delim_attrindex &&
		...
		if (foundopenbracket && !foundclosebracket) {
			...
			if (thiskeydoc !== undefined && "constraint" in thiskeydoc) {
				...
				if (thiskeydoc.constraint instanceof Array) {
					...
					for (const val of thiskeydoc.constraint) {
						result.push({
							label: val,
							kind: CompletionItemKind.EnumMember,
							data: "keywordvalue",
			...
			if (parsed[j][0].l == ld.cls_langindex && parsed[j][0].s == ld.cls_keyword_attrindex) {
				...
				for (let k = 3; k < parsed[j].length; k++) {
					if (parsed[j][k].l == ld.cls_langindex && parsed[j][k].s == ld.cls_keyword_attrindex) {
						...
						const keytext = doc
							.getText(Range.create(j, parsed[j][k].p, j, parsed[j][k].p + parsed[j][k].c))
							.toLowerCase();
						if (keytext === "xmlnamespace") {
							// An XMLNamespace is defined
							xmlns = doc.getText(
								Range.create(j, parsed[j][k + 2].p + 1, j, parsed[j][k + 2].p + parsed[j][k + 2].c - 1),
							);
							break;
						}
					}
				...
				if (schema !== undefined) {
					...
					for (let xmlline = xmlstartline; xmlline <= params.position.line; xmlline++) {
						...
						for (let xmltkn = 0; xmltkn <= endtkn; xmltkn++) {
							if (
								parsed[xmlline][xmltkn].l == ld.xml_langindex &&
								parsed[xmlline][xmltkn].s == ld.xml_tagdelim_attrindex
							) {
					...
					if (prevline.slice(-1) === " ") {
						...
						for (let tkn = parsed[params.position.line].length - 1; tkn >= 0; tkn--) {
							...
							if (
								parsed[params.position.line][tkn].l == ld.xml_langindex &&
								parsed[params.position.line][tkn].s == ld.xml_attr_attrindex
							) {
		...
		if (thistoken === 0) {
			...
			for (const dir of preprocessorDirectives) {
		...
		if (storageObjKey != "") {
			...
			if (keywords) {
				...
				result.push(
					...keywords
						.filter((keydoc) => keydoc.name != "Name")
						.map((keydoc) => {
							let doctext = keydoc.description;
							if (doctext === undefined) {
								doctext = "";
...
function* completeStatment(prefix: string): Generator<CompletionItem> {
