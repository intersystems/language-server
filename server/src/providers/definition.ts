			...
			if (((parsed[params.position.line][i].l == ld.cls_langindex && parsed[params.position.line][i].s == ld.cls_clsname_attrindex) ||
				...
				if (queryrespdata !== undefined) {
					if (Array.isArray(queryrespdata?.data?.result?.content) && queryrespdata.data.result.content.length > 0) {
						...
						if (result.Stub !== "") {
							// This is a method generated by member inheritance, so we need to get the Origin of the "parent" member to go to it

							originclass = "";
							const [stubMember, , stubMemberType] = result.Stub.split(".");
							let stubQuery;
							switch (stubMemberType) {
								case "i":
									// This is a method generated from an index
									memberKeywords = "Index";
									stubQuery = "SELECT Origin FROM %Dictionary.CompiledIndex WHERE Name = ? AND Parent = ?";
									break;
								case "q":
									// This is a method generated from a query
									memberKeywords = "Query";
									stubQuery = "SELECT Origin FROM %Dictionary.CompiledQuery WHERE Name = ? AND Parent = ?";
									break;
								case "a":
									// This is a method generated from a property
									memberKeywords = "Property|Relationship";
									stubQuery = "SELECT Origin FROM %Dictionary.CompiledProperty WHERE Name = ? AND Parent = ?";
							}
							if (stubQuery) {
								...
								if (Array.isArray(stubrespdata?.data?.result?.content) && stubrespdata.data.result.content.length > 0) {
									...
									membernameinfile = quoteUDLIdentifier(stubMember, 1);
								}
							}
						}
					}
				...
				if (
					...
					if (normalizedname !== "") {
						...
						let querystr = "SELECT Origin FROM %Dictionary.CompiledMethod WHERE Parent = ? AND name = ? UNION ALL ";
