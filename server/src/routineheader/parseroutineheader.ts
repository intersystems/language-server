...
import { compressedline, routineheaderinfotype } from "../types";
import assert = require("assert");
import { validInRoutineName, isValidRoutineName, isWhitespace, cos_attribute_name, cos_attribute_delimiter, cos_attribute_routine, keywordstype, cos_attribute_command } from "./routineheaderutils";
...
function colorRoutineLineImpl(linesource: LineSource): routineheaderinfotype {
    ...
    linesource.commitToken(cos_attribute_command);
    ...
    if (!linesource.ended() && linesource.currentChar() === '[') {
        ...
        linesource.commitToken(cos_attribute_delimiter);
...
function parseRoutineName(linesource: LineSource): string {
    ...
    if (!isValidRoutineName(routinename)) {
        ...
        linesource.commitToken(cos_attribute_routine);
    }
